        ...
        public void Check()
        {
            foreach(var file in Files)
            {
                ...
                if (file.Contains("{n}"))
                {
                    ...
                    {
                        str = file;
                        var fullPath = System.IO.Path.Combine(Folder, str.Replace("{Name}", xzc.XZCMC).Replace("{Code}", xzc.XZCDM));
                        Check(fullPath);
                    }
                }
                else
                {
                    var fullPath = System.IO.Path.Combine(Folder, str.Replace("{Name}", ParameterManager.District).Replace("{Code}", ParameterManager.Code));
                    Check(fullPath);
                }
            }
        }
