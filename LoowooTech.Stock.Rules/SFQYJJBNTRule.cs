...
{
    public class SFQYJJBNTRule:ArcGISBaseTool,Models.IRule
    {
        public override string RuleName { get { return "示范区永久基本农田一致性检查"; } }
        public override string ID { get { return "6002"; } }
        public override CheckProject2 CheckProject { get { return CheckProject2.村规划数据库与乡规划数据库示范区永久基本农田空间范围是否一致; } }
        ...
        private List<Question2> CheckArea(List<string> list,string mdbFile,string id ,CheckProject2 checkProject2,double? relative,double? absolute)
        {
            var questions = new List<Question2>();
                ...
                foreach(var xzqdm in list)
                {
                    ...
                    if (cghyt == null)
                    {
                        questions.Add(new Question2
                        {
                            ...
                            Name = RuleName,
                            CheckProject = checkProject2,
                            Description = string.Format("无法获取行政区代码【{0}】的村规划中示范区永久基本农田的要素类", xzqdm)
                        });
                        continue;
                    }
                    ...
                    if (xghyt == null)
                    {
                        questions.Add(new Question2
                        {
                            ...
                            Name = RuleName,
                            CheckProject = checkProject2,
                            Description = string.Format("无法获取行政区代码【{0}】的乡规划中示范区永久基本农田的要素类", xzqdm)
                        });
                        continue;
                    }
                    ...
                    if (flag == false)
                    {
                        questions.Add(new Question2
                        {
                            ...
                            Name = RuleName,
                            CheckProject = checkProject2,
                            Description = string.Format("行政区代码【{0}】的村规划示范区永久基本农田面积与乡规划示范区永久基本农田面积不一致", xzqdm)
                        });
                    }
